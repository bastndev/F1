...
type CliSession = CliSessionSnapshot & {
...
export class CliSessionManager implements vscode.Disposable {
	...
	private startPtyHost(session: CliSession, command: string, args: string[]) {
		...
		host.on('message', (message: PtyHostMessage) => {
			this.handlePtyHostMessage(session, message);
		});
		host.on('error', (error) => {
	...
	private handlePtyHostMessage(session: CliSession, message: PtyHostMessage) {
	...
	public postState() {
		this.postMessage({
			...
			sessions: [...this.sessions.values()].map((session) => this.toSnapshot(session))
		});
	}
	...
	private writeInput(sessionId: string, data: string) {
		...
		session.process?.send?.({ type: 'input', data });
	}
	...
	private toSnapshot(session: CliSession): CliSessionSnapshot {
		return {
			id: session.id,
			label: session.label,
